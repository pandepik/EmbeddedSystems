...
#define UART_IFLS_RX1_8         0x00000000  // RX FIFO >= 1/2 full
#define UART_IFLS_TX1_8         0x00000000  // TX FIFO <= 1/2 full
...
void UART_Init(void){
  ...
  UART1_LCRH_R = 0x70;					//(UART_LCRH_WLEN_8|UART_LCRH_FEN);
  ...
  GPIO_PORTC_AMSEL_R = 0;               // disable analog functionality on PC4-5
                                        // UART0=priority 2
  NVIC_PRI1_R = 0x00A00000; // bits 13-15
  NVIC_EN0_R = 0x40;           // enable interrupt 5 in NVIC
...
void UART1_Handler(void){    //0x10
//	while((UART1_FR_R&0x0010) != 0){}
	char c;
	c = UART_InChar();
	FIFO_Put(c);//UART1_DR_R);
	UART1_ICR_R = 0x10;//UART_ICR_RXIC;
}
/*
//------------UART_OutString------------
