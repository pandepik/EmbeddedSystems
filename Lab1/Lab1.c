...
// operations to be executed over and over go here - Using bit-wise logic operations is efficient
		stat = ((((~GPIO_PORTF_DATA_R)&0x10)>> 1) & (((~GPIO_PORTF_DATA_R) & 0x01)<<3));  //Turns green if both closed   
		stat1 = ((GPIO_PORTF_DATA_R & 0x10)>>3) & ((GPIO_PORTF_DATA_R & 0x01)<<1); 				//Turns red if both open
		stat2 = ((GPIO_PORTF_DATA_R & 0x10)>>3) ^ ((GPIO_PORTF_DATA_R & 0x01)<<1); 				//Turns red if either open 
		GPIO_PORTF_DATA_R = stat | stat1 | stat2; 																				//Sets the LED
