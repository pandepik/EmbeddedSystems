...
uint16_t StTextColor = ST7735_CYAN;
...
void ST7735_MoveSBall(int16_t x, int16_t y, int16_t n_x, int16_t n_y, int16_t r, uint16_t color){
	ST7735_FillRect(x, y, 5, 5, 0);
	ST7735_FillRect(n_x, n_y, 5, 5, color);
	
}

void ST7735_MoveBPaddle(int16_t x, int16_t y, int16_t n_x, uint16_t length, int16_t sq, uint16_t color){
	if(n_x < 0 || n_x+length > 127 || n_x == x){return;}
	ST7735_FillRect(x, y, length+1, sq, 0);
	ST7735_FillRect(n_x, y, length, sq, color);
}

void ST7735_MoveRPaddle(int16_t x, int16_t y, int16_t n_y, uint16_t length, int16_t sq, uint16_t color){
	if(n_y < 0 || n_y + length > 160 || n_y == y){return;}
	ST7735_FillRect(x, y, sq, length+1, 0);
	ST7735_FillRect(x, n_y, sq, length, color);
}

// (x, y) == center of the cirlce
// r == radius of the circle, how many pixels from the center
void ST7735_DrawDiamond(int16_t x, int16_t y, int16_t r, uint16_t color){
	//rudimentary clipping
	int16_t cent_x = x;
	int16_t cent_y = y;
	if((x-r) >= _width && (y-r) >= _height) {return;}		
	ST7735_DrawPixel(x, y, color); //draw the center pixel
	while(y < (cent_y+r)){
		y++;
		while(y != cent_y){								//Draw diagonal bot right corner
			x++;
			y--;
			ST7735_DrawPixel(x, y, color);
		}
		while(x != cent_x){								//Draw diagonal top right corner
			x--;
			y--;
			ST7735_DrawPixel(x, y, color);
		}
		while(y != cent_y){								//Draw diagonal top left corner
			x--;
			y++;
			ST7735_DrawPixel(x, y, color); 
		}
		while(x != cent_x){								//Draw diagonal bot left coner
			x++;
			y++;
			ST7735_DrawPixel(x, y, color);
		}
...
void ST7735_DrawCircle(int16_t x, int16_t y, int16_t r, uint16_t color){
	int16_t ix, iy, i;
	ST7735_DrawPixel(x, y, color);
	if(color != 0){
		for(i=r; i>0;i--){
			for(ix=x-i; ix!=x+i+1;ix++){
				ST7735_DrawPixel(ix, y-square(i*i-(ix-x)*(ix-x)), 0xF800);
				ST7735_DrawPixel(ix, y+square(i*i-(ix-x)*(ix-x)), 0xF800);
			}
		}
		for(i=r; i>0;i--){
			for(iy=y-i; iy!=y+i+1;iy++){
				ST7735_DrawPixel(x-square(i*i-(iy-y)*(iy-y)), iy, color);
				ST7735_DrawPixel(x+square(i*i-(iy-y)*(iy-y)), iy, color);
			}
		}
	}
		...
		for(i=r; i>0;i--){
			for(ix=x-i; ix!=x+i+1;ix++){
				ST7735_DrawPixel(ix, y-square(i*i-(ix-x)*(ix-x)), color);
				ST7735_DrawPixel(ix, y+square(i*i-(ix-x)*(ix-x)), color);
			}
		}
		for(i=r; i>0;i--){
...
void ST7735_DrawECircle(int16_t x, int16_t y, int16_t r, uint16_t color){
	int16_t ix, iy;
		for(ix=x-r; ix!=x+r+1;ix++){
			ST7735_DrawPixel(ix, y-square(r*r-(ix-x)*(ix-x)), color);
			ST7735_DrawPixel(ix, y+square(r*r-(ix-x)*(ix-x)), color);
		}
		for(iy=y-r; iy!=y+r+1;iy++){
			ST7735_DrawPixel(x-square(r*r-(iy-y)*(iy-y)), iy, color);
			ST7735_DrawPixel(x+square(r*r-(iy-y)*(iy-y)), iy, color);
		}
}

void ST7735_MoveCBall(int16_t x, int16_t y, int16_t n_x, int16_t n_y, int16_t r, uint16_t color){
	ST7735_DrawCircle(x, y, r, 0);
	ST7735_DrawCircle(n_x, n_y, r, color);
}	

uint32_t square(uint32_t x){
	uint32_t res;
	for(res=0; res*res<x; res++){}
	if(res*res-x<x-(res-1)){
		return res; 
	}else{
	return (res-1);
	}
...
uint32_t ST7735_MoveCursor(uint32_t x, uint32_t y, uint32_t size, uint32_t dir, uint32_t color){
	int i;
	switch(dir){
		case 0:
			if(y == 26 || (y == 62 && (x == 14 || x == 30 || x == 94 || x == 110))){}
			else{
			ST7735_DrawECircle(x, y, size, 0xFFFF);
			ST7735_DrawECircle(x, y-18, size, color);
			i = 1;
			}
			break;
		case 1:
			if(y == 134 || (y == 98 && (x == 14 || x == 30 || x == 94 || x == 110))){}
			else{
			ST7735_DrawECircle(x, y, size, 0xFFFF);
			ST7735_DrawECircle(x, y+18, size, color);
			i = 1;
			...
			break;
		case 2:
			if(x == 14 || (x == 46 && (y == 26 || y == 44 || y == 116 || y== 134))){}
			else{
			ST7735_DrawECircle(x, y, size, 0xFFFF);
			ST7735_DrawECircle(x-16, y, size, color);
			i = 1;
			}
			break;
		case 3:
			if(x == 110 || (x == 78 && (y == 26 || y == 44 || y == 116 || y== 134))){}
			else{
			ST7735_DrawECircle(x, y, size, 0xFFFF);
			ST7735_DrawECircle(x+16, y, size, color);
			i = 1;
...
void ST7735_MoveCursor2(uint32_t x, uint32_t y, uint32_t size, uint32_t dir, uint32_t color){
	ST7735_DrawECircle(x, y, size, 0xFFFF);
	switch(dir){
		case 0:ST7735_DrawECircle(x, y-(18*2), size, color);
			break;
		case 1:ST7735_DrawECircle(x, y+(18*2), size, color);
			break;
		case 2:ST7735_DrawECircle(x-(16*2), y, size, color);
			break;
		case 3:ST7735_DrawECircle(x+(16*2), y, size, color);
			break;
	}
}

void ST7735_MoveCursorT(uint32_t x, uint32_t y, uint32_t size, uint32_t dir, uint32_t color){
	ST7735_DrawECircle(x, y, size, 0xFFFF);
	switch(dir){
		case 0:ST7735_DrawECircle(x, y-(18), size, color);
			break;
		case 1:ST7735_DrawECircle(x, y+(18), size, color);
			break;
		case 2:ST7735_DrawECircle(x-(16), y, size, color);
			break;
		case 3:ST7735_DrawECircle(x+(16), y, size, color);
			break;
	}
}

void ST7735_DrawHRect(int16_t x, int16_t y, int16_t width, int16_t height, int16_t thick, uint32_t color){
	int i, a;
	//Draw horizontal lines
	for(a = y; a<y+thick; a++){
		for(i = x; i<x+width; i++){
			ST7735_DrawPixel(i, a, color);
		}
	}
	for(a = y+height; a<y+height+thick; a++){
		for(i = x; i<x+width; i++){
			ST7735_DrawPixel(i, a, color);
		}
	}
	...
	for(a = x; a<x+thick; a++){
		for(i = y; i<y+height; i++){
			ST7735_DrawPixel(a, i, color);
		}
	}
	for(a = x+width; a<x+width+thick; a++){
		for(i = y; i<y+height; i++){
			ST7735_DrawPixel(a, i, color);
		}
	}			
